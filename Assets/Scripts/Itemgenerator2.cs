using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Itemgenerator2 : MonoBehaviour {

    public GameObject carPrefab;   //carPrefabを入れる
    public GameObject coinPrefab;  //coinPrefabを入れる
    public GameObject conePrefab;  //cornPrefabを入れる
    private int startPos = 80;     //スタート地点
    private int goalPos = 360;     //ゴール地点
    private float posRange = 3.4f; //アイテムを出すx方向の範囲

    private GameObject unitychan;  //UnityChanのオブジェクト
    private float position_z;　    //UnityChanのz座標を保存

    // Start is called before the first frame update
    void Start()
    {
        //UnityChanのオブジェクトを取得
        this.unitychan = GameObject.Find("unitychan");
        //初期位置のz座標を保存
        position_z = this.unitychan.transform.position.z;
    }

    // Update is called once per frame
    void Update() { 

        //UnityChanが15m移動するごとに起動
        if (this.unitychan.transform.position.z - position_z > 15f) {

            //どのアイテムを出すのかをランダムに設定
            int num = Random.Range(1, 11);
            //道路の80~360m以内にアイテムを生成するように制限
            if ((unitychan.transform.position.z + 50f) > startPos && (unitychan.transform.position.z + 50f) < goalPos) { 
                if (num <= 2) {
                    //コーンをx軸方向に一直線に生成
                    for (float j = -1; j <= 1; j += 0.4f) {
                        GameObject cone = Instantiate(conePrefab);
                        cone.transform.position = new Vector3(4 * j, cone.transform.position.y, unitychan.transform.position.z + 50f);
                    }
                }
                else {

                    //レーンごとにアイテムを生成
                    for (int j = -1; j <= 1; j++) {
                        //アイテムの種類を決める
                        int item = Random.Range(1, 11);
                        //アイテムを置くZ座標のオフセットをランダムに設定
                        int offsetZ = Random.Range(-5, 6);
                        //60%コイン配置:30%車配置:10%何もなし
                        if (1 <= item && item <= 6) {
                            //コインを生成
                            GameObject coin = Instantiate(coinPrefab);
                            coin.transform.position = new Vector3(posRange * j, coin.transform.position.y, unitychan.transform.position.z + 50f + offsetZ);
                        }
                        else if (7 <= item && item <= 9) {
                            //車を生成
                            GameObject car = Instantiate(carPrefab);
                            car.transform.position = new Vector3(posRange * j, car.transform.position.y, unitychan.transform.position.z + 50f + offsetZ);
                        }
                    }
                }
            }
            //z座標を更新
            position_z = unitychan.transform.position.z;
        }

    }
}
